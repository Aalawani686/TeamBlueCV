...
TargetProcessing targetProcessor(480, 320, 240);
...
void run()
{
  ...
  { // while the horizontal azimuth of first camera is greater than error constant
    ...
    sideCam >> sideImg;
    alignCross = targetDetector.processImage(sideImg, 12, hueMin, hueMax, valMin, valMax); // make the image of horizontal detection a target
  ...
  while (highGoalAz > EPSILON) // loop until we are aligned with the high goal
  {
    ...
    backCam >> backImg;
    alignCross = targetDetector.processImage(backImg, 12, hueMin, hueMax, valMin, valMax);
    if(alignCross != nullptr)
    {
      targetProcessor.loadTarget(*alignCross, 20.32);
    ...
    // look for high goal
    sideCam >> sideImg;
    highGoal = targetDetector.processImage(sideImg, 8, hueMin, hueMax, valMin, valMax);
    if(highGoal != nullptr)
    {
      ...
      if(fabs(highGoalAlt) + CAM_ANGLE > HighMedAvg)){
        msg << "foundHighGoal:1;" << "highGoalDist:" << highGoalDist << ";highGoalAz:" << highGoalAz << ";highGoalAlt:" << highGoalAlt + CAM_ANGLE;
        networkController.sendMessage(msg.str());
        msg.str("");
      }
    }
    ...
    if(!foundParkLoc)
    {
      ...
      parkCross = targetDetector.processImage(sideImg, 8,hueMin, hueMax, valMin, valMax);
  ...
  while (parkCrossDist > 20)
  {
    networkController.waitForPing();
    backCam >> backImg;
    parkCross = targetDetector.processImage(backImg, 12, hueMin, hueMax, valMin, valMax);
    if(parkCross != nullptr)
    {
